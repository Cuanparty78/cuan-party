...
class _ChatPageState extends State<ChatPage> {
  ...
  void _friendRequests() {
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(26))),
      builder: (sheetContext) => StatefulBuilder(builder: (_, modalSetState) => SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(20, 16, 20, 20), child: Column(mainAxisSize: MainAxisSize.min, children: [
        Row(children: [const Expanded(child: Text('Permintaan Teman', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w900))), IconButton(onPressed: () => Navigator.pop(sheetContext), icon: const Icon(Icons.close))]),
        const SizedBox(height: 8),
        if (_requests.isEmpty) const Padding(padding: EdgeInsets.all(24), child: Text('Tidak ada permintaan teman.', style: TextStyle(color: _C.muted)))
        else ..._requests.map((request) => Padding(padding: const EdgeInsets.only(bottom: 10), child: _LuxuryCard(child: Row(children: [const CircleAvatar(backgroundColor: _C.gold2, child: Icon(Icons.person, color: _C.brown)), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(request['name']!, style: const TextStyle(fontWeight: FontWeight.w900)), Text('ID: ${request['id']}', style: const TextStyle(color: _C.muted, fontSize: 11))])), OutlinedButton(onPressed: () { setState(() {}); modalSetState(() { _requests.remove(request); }); }, child: const Text('Tolak')), const SizedBox(width: 6), FilledButton(style: FilledButton.styleFrom(backgroundColor: _C.brown), onPressed: () { setState(() {}); modalSetState(() { _requests.remove(request); }); _showMessage(context, '${request['name']} sekarang menjadi teman.'); }, child: const Text('Terima'))]))),
      ]))),
    );
  }
  ...
  Widget build(BuildContext context) {
    return SafeArea(child: Scaffold(backgroundColor: _C.bg, body: Column(children: [
      ...
      const SizedBox(height: 8),
      Expanded(child: ListView(padding: const EdgeInsets.fromLTRB(20, 4, 20, 20), children: [for (final item in _items) Padding(padding: const EdgeInsets.only(bottom: 10), child: _LuxuryCard(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => PrivateChatPage(userName: item.name, userId: item.name == 'Nana' ? '1000002' : item.name == 'Raka' ? '1000003' : '1000004')), padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 13), child: Row(children: [CircleAvatar(radius: 27, backgroundColor: _C.gold2, child: Icon(item.name == 'System' ? Icons.lock_rounded : item.name.startsWith('Official') ? Icons.workspace_premium_rounded : Icons.person, color: _C.brown)), const SizedBox(width: 13), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item.name, style: const TextStyle(color: _C.text, fontWeight: FontWeight.w900, fontSize: 15)), const SizedBox(height: 4), Text(item.preview, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: _C.muted, fontSize: 12))])), Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text(item.time, style: const TextStyle(color: _C.muted, fontSize: 11)), if (item.unread > 0) ...[const SizedBox(height: 6), CircleAvatar(radius: 11, backgroundColor: _C.gold, child: Text('${item.unread}', style: const TextStyle(color: Colors.white, fontSize: 10, fontWeight: FontWeight.w800)))]])])))])),
    ])));
  }
}
