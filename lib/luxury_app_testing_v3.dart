...
class _RoomDetailPageState extends State<RoomDetailPage> {
  ...
  String _firstGiftTarget() {
    for (final name in _seatNames) {
      if (name != null && name != 'Jep') return name;
    }
    return 'QueenA';
  }
            ...
            builder: (_, coins, __) => Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (value) {
                    if (value != null) setState(() => _giftTarget = value);
                  },
                ),
                const SizedBox(height: 14),
                GridView.count(
                  ...
                  children: [
                    for (final gift in gifts)
                      _GiftButton(
                        ...
                        cost: gift.$3,
                        onSend: () {
                          if (testerCoinBalance.value < gift.$3) {
                            _showMessage(context, 'Coin tidak cukup.');
                            return;
                          }
                          _setTesterCoins(
                              testerCoinBalance.value - gift.$3);
                          _addRoomEvent(
                            _RoomEvent.gift(
                              'Jep',
                              _giftTarget,
                          ...
                          _addTesterTransaction('Gift ${gift.$1} → $_giftTarget -${_formatCoins(gift.$3)} Coin');
                          Navigator.pop(sheetContext);
                          _showGiftSuccess(context, gift.$1, gift.$2, _giftTarget, gift.$3);
                        },
                      ),
                  ],
                ),
  ...
  void _showGiftSuccess(BuildContext context, String giftName, IconData icon, String target, int cost) {
    showDialog(
      context: context,
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            Text(giftName, style: const TextStyle(color: _C.text, fontSize: 22, fontWeight: FontWeight.w900)),
            const SizedBox(height: 6),
            Text('Gift terkirim ke $target', style: const TextStyle(color: _C.brown2)),
            const SizedBox(height: 4),
            Text('-${_formatCoins(cost)} Coin', style: const TextStyle(color: _C.gold, fontWeight: FontWeight.w900)),
          ],
        ),
      ),
    );
...
void _showTestingExchange(BuildContext context) {
  showDialog(
    ...
    builder: (dialogContext) => AlertDialog(
      ...
      content: const Text('Untuk testing: 1,000 Coin = 100 Diamond. Coin akan dipotong langsung.'),
      actions: [
        TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('Batal')),
        FilledButton(
          onPressed: () {
            if (testerCoinBalance.value < 1000) {
              Navigator.pop(dialogContext);
              _showMessage(context, 'Coin tidak cukup.');
              return;
            }
            _setTesterCoins(testerCoinBalance.value - 1000);
            _addTesterTransaction('Exchange -1,000 Coin → +100 Diamond');
            Navigator.pop(dialogContext);
            _showMessage(context, 'Exchange berhasil: +100 Diamond.');
          },
          child: const Text('Exchange 1,000'),
        ),
      ],
    ),
  );
