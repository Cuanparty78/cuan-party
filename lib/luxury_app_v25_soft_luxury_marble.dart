...
class _PalaceBackgroundPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // CUAN PARTY background: ivory marble first, royal details second.
    // No hard columns, no orange wall, no literal palace wallpaper.
    final base = Paint()
    ...
    // Soft champagne light, kept behind the UI.
    final glow = Paint()
      ..shader = RadialGradient(
        center: const Alignment(0, -.35),
        radius: 1.0,
        colors: [
          const Color(0xFFFFE9B0).withOpacity(.46),
          Colors.transparent,
        ],
      ).createShader(Rect.fromLTWH(0, 0, w, h * .72));
    canvas.drawRect(Rect.fromLTWH(0, 0, w, h * .72), glow);

    // Natural-looking marble veins.
    final vein = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.15
      ..color = const Color(0x249A764A);
    for (int i = -3; i < 6; i++) {
      final x = i * 105.0;
      final path = Path()..moveTo(x, -20);
      path.cubicTo(x + 70, h * .14, x - 40, h * .30, x + 60, h * .47);
      path.cubicTo(x + 125, h * .59, x + 10, h * .74, x + 85, h + 20);
    ...
    final vein2 = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = .65
      ..color = const Color(0x187B5A35);
    for (int i = -2; i < 5; i++) {
      final x = i * 145.0 + 35;
      final path = Path()..moveTo(x, h * .08);
      path.cubicTo(x - 45, h * .24, x + 80, h * .38, x + 5, h * .56);
      path.cubicTo(x - 55, h * .70, x + 65, h * .84, x + 25, h);
      canvas.drawPath(path, vein2);
    }

    // Very faint royal arch silhouette — an atmosphere, not a building.
    final arch = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 2.2
      ..color = const Color(0x28C79A50);
    final archPath = Path()
      ..moveTo(w * .12, h * .70)
      ..lineTo(w * .12, h * .30)
      ..quadraticBezierTo(w * .12, h * .08, w * .5, h * .055)
      ..quadraticBezierTo(w * .88, h * .08, w * .88, h * .30)
      ..lineTo(w * .88, h * .70);
    canvas.drawPath(archPath, arch);

    // Subtle gold filigree at the four corners.
    final ornament = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.6
      ..color = const Color(0x55C59A52);
    _cornerOrnament(canvas, Offset(18, 92), 1, 1, ornament);
    _cornerOrnament(canvas, Offset(w - 18, 92), -1, 1, ornament);
    _cornerOrnament(canvas, Offset(18, h - 92), 1, -1, ornament);
    _cornerOrnament(canvas, Offset(w - 18, h - 92), -1, -1, ornament);

    // Soft bottom marble medallion.
    final center = Offset(w * .5, h * .93);
    final floor = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.8
      ..color = const Color(0x3AC0924A);
    canvas.drawOval(Rect.fromCenter(center: center, width: w * 1.18, height: h * .18), floor);
    canvas.drawOval(Rect.fromCenter(center: center, width: w * .74, height: h * .105), floor);

    final star = Path();
    for (int i = 0; i < 16; i++) {
      final angle = -math.pi / 2 + i * math.pi / 8;
      final r = i.isEven ? w * .12 : w * .045;
      final pt = Offset(center.dx + math.cos(angle) * r, center.dy + math.sin(angle) * r * .42);
      if (i == 0) {
    ...
    // Tiny champagne sparkle points for depth.
    final sparkle = Paint()..color = const Color(0x66C99B4F);
    for (final p in [
      Offset(w * .10, h * .18),
      Offset(w * .87, h * .22),
      Offset(w * .17, h * .76),
      ...
      canvas.drawCircle(p, 1.8, sparkle);
    }
  }

  void _cornerOrnament(Canvas canvas, Offset origin, double sx, double sy, Paint paint) {
    final path = Path()
      ..moveTo(origin.dx, origin.dy)
      ..cubicTo(origin.dx + 18 * sx, origin.dy, origin.dx + 30 * sx, origin.dy + 4 * sy, origin.dx + 42 * sx, origin.dy + 18 * sy)
      ..cubicTo(origin.dx + 26 * sx, origin.dy + 8 * sy, origin.dx + 18 * sx, origin.dy + 25 * sy, origin.dx + 12 * sx, origin.dy + 42 * sy);
    canvas.drawPath(path, paint);
    canvas.drawCircle(Offset(origin.dx + 13 * sx, origin.dy + 13 * sy), 2.2, paint);
  }
