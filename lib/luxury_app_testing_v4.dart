...
class _RoomDetailPageState extends State<RoomDetailPage> {
  ...
  void _showGifts() {
    const gifts = [
      ('Rose', Icons.local_florist_rounded, 100),
      ('Heart', Icons.favorite_rounded, 500),
      ('Crown', Icons.workspace_premium_rounded, 2500),
      ('Diamond', Icons.diamond_rounded, 10000),
    ];
                ...
                builder: (_, coins, __) {
                  return Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 6),
                      Text(
                        'Coin tersedia: ${_formatCoins(coins)}',
                      ...
                      DropdownButtonFormField<String>(
                        ...
                        items: [
                          ...
                          for (final name in _seatNames.where(
                            (name) => name != null && name != 'Jep',
                          ).cast<String>())
                            DropdownMenuItem(
                              value: name,
                              child: Text(name),
                        ...
                        children: [
                          for (final gift in gifts)
                            _GiftButton(
                              ...
                              cost: gift.$3,
                              onSend: () {
                                final totalCost = gift.$3 * quantity;
                                if (testerCoinBalance.value < totalCost) {
                                  _showMessage(
                                    context,
                                    'Coin tidak cukup. Butuh ${_formatCoins(totalCost)} Coin.',
                                  );
                                  return;
                                }

                                _setTesterCoins(
                                  testerCoinBalance.value - totalCost,
                                );

                                final isSelfGift = target == 'Jep';
                                if (isSelfGift) {
                                  _addTesterDiamonds(totalCost);
                                }
                                ...
                                _addTesterTransaction(
                                  'Gift ${gift.$1} x$quantity → $target '
                                  '-${_formatCoins(totalCost)} Coin'
                                  '${isSelfGift ? ' / +${_formatCoins(totalCost)} Diamond' : ''}',
                                );
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                shape: BoxShape.circle,
                gradient: const LinearGradient(colors: [_C.gold2, _C.gold]),
                boxShadow: [BoxShadow(color: _C.gold.withOpacity(.25), blurRadius: 12)],
              ),
              child: Icon(icon, color: _C.brown, size: 29),
            ),
            ...
            Text(
              '${_formatCoins(cost)} Coin',
              style: const TextStyle(color: _C.brown2, fontSize: 9),
            ),
          ],
        ),
      ),
...
void _showTestingExchange(BuildContext context) {
  ...
  const diamondAmount = 100;

  showDialog(
    context: context,
    builder: (dialogContext) => AlertDialog(
      title: const Text('Exchange Testing'),
      content: ValueListenableBuilder<int>(
        valueListenable: testerDiamondBalance,
        builder: (_, diamonds, __) => ValueListenableBuilder<int>(
          valueListenable: testerCoinBalance,
          builder: (_, coins, __) => Column(
            ...
            children: [
              ...
              const SizedBox(height: 8),
              const Text('1,000 Coin ↔ 100 Diamond'),
              const SizedBox(height: 12),
              Text('Coin: ${_formatCoins(coins)}'),
              Text('Diamond: ${_formatCoins(diamonds)}'),
            ],
          ),
        ),
      ),
      actions: [
        ...
        FilledButton(
          onPressed: () {
            if (testerCoinBalance.value < coinAmount) {
              Navigator.pop(dialogContext);
              _showMessage(context, 'Coin tidak cukup.');
              return;
            }

            _setTesterCoins(testerCoinBalance.value - coinAmount);
            _addTesterDiamonds(diamondAmount);
            _addTesterTransaction(
              '-${_formatCoins(coinAmount)} Coin → +${_formatCoins(diamondAmount)} Diamond',
            );
            Navigator.pop(dialogContext);
            _showMessage(
              context,
              'Exchange berhasil: +${_formatCoins(diamondAmount)} Diamond.',
            );
          },
          child: const Text('Coin → Diamond'),
        ),
        FilledButton.tonal(
          onPressed: () {
            if (testerDiamondBalance.value < diamondAmount) {
              Navigator.pop(dialogContext);
              _showMessage(context, 'Diamond tidak cukup.');
              return;
            }

            _setTesterDiamonds(testerDiamondBalance.value - diamondAmount);
            _addTesterCoins(coinAmount);
            _addTesterTransaction(
              '-${_formatCoins(diamondAmount)} Diamond → +${_formatCoins(coinAmount)} Coin',
            );
            Navigator.pop(dialogContext);
            _showMessage(
              context,
              'Exchange berhasil: +${_formatCoins(coinAmount)} Coin.',
            );
          },
          child: const Text('Diamond → Coin'),
        ),
      ],
    ),
  );
