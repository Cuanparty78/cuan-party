...
class _RoomDetailPageState extends State<RoomDetailPage> {
  ...
  final List<_RoomMessage> _messages = [
    _RoomMessage('Raka', 'Suara kamu enak banget! ❤️'),
    _RoomMessage('Nana', 'Makasihh 😍'),
    _RoomMessage('Dimas', 'Request lagu dong'),
    _RoomMessage('Salsa', 'Boleh banget!'),
  ];
  bool _micOn = false;
  bool _joined = false;
  ...
  void initState() {
    ...
    _seatNames = List<String?>.generate(
      8,
      (i) => i < widget.seats.clamp(0, 8) ? names[i] : null,
    );
  }
  ...
  void _toggleMic() {
    ...
    _showMessage(context, _micOn ? 'Mic ON (mode tester).' : 'Mic OFF.');
  }
  ...
  void _sendMessage() {
    ...
    setState(() {
      _messages.add(_RoomMessage('Jep', value));
  ...
  void _takeSeat(int index) {
    ...
    _showMessage(context, 'Kamu duduk di seat ${index + 1}.');
  }
          ...
          child: Column(
            ...
            children: [
              ...
              GridView.count(
                ...
                crossAxisSpacing: 12,
                children: [
                  _GiftButton('Rose', Icons.local_florist_rounded),
                  _GiftButton('Heart', Icons.favorite_rounded),
                  _GiftButton('Crown', Icons.workspace_premium_rounded),
                  _GiftButton('Diamond', Icons.diamond_rounded),
                ],
              ),
            ],
          ),
            ...
            child: ListView(
              ...
              children: [
                    ...
                    children: [
                      Row(
                        children: [
                          const CircleAvatar(
                            radius: 31,
                            backgroundColor: _C.gold2,
                            child: Icon(Icons.person, color: _C.brown, size: 34),
                          ),
                          const SizedBox(width: 12),
                          Expanded(
                            child: Column(
                            ...
                            child: Row(
                              children: [
                                Icon(
                                  Icons.circle,
                                  size: 9,
                                  color: _joined ? Colors.green : _C.muted,
                                ),
                                const SizedBox(width: 6),
                                Text(
                                  _joined ? 'Joined' : 'Preview',
                                  style: const TextStyle(
                                    color: _C.text,
                      ...
                      SizedBox(
                        ...
                        child: FilledButton(
                          ...
                          child: Text(_joined ? 'Leave Room' : 'Join Room'),
                        ),
                      ),
                    ],
                  ),
                ...
                GridView.builder(
                  ...
                  itemBuilder: (_, i) {
                    final name = _seatNames[i];
                    final occupied = name != null;
                    return _LuxuryCard(
                      onTap: occupied && name != 'Jep' ? null : () => _takeSeat(i),
                  ...
                  child: Column(
                    children: [
                      for (final message in _messages)
                        _ChatLine(message.name, message.text),
                    ],
                  ),
                ),
              ],
...
class _RoomMessage {
  final String name;
  final String text;
  const _RoomMessage(this.name, this.text);
}

class _GiftButton extends StatelessWidget {
  final String title;
  final IconData icon;
  const _GiftButton(this.title, this.icon);
  ...
  Widget build(BuildContext context) {
    return InkWell(
      ...
      onTap: () {
        ...
        _showMessage(context, '$title gift terkirim (mode tester).');
      },
      child: Container(
        decoration: BoxDecoration(
          color: _C.surface2,
