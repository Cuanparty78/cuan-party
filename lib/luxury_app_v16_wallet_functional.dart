                    ...
                    child: _LuxuryCard(
                      child: Row(children: [
                        ...
                        FilledButton(
                          ...
                          onPressed: () {
                            modalSetState(() => _ownerJoinRequests.remove(request));
                            _showMessage(context, '${request['name']} diterima menjadi member Family.');
                          },
...
class _WalletPageState extends State<WalletPage> {
  int _tab = 0;
  int _exchangeMode = 0; // 0 = Coin -> Diamond, 1 = Diamond -> Coin
  final _exchangeController = TextEditingController();

  final List<Map<String, String>> _packages = const [
    {'coins': '10,000', 'amount': '10000', 'price': 'Rp 1.000'},
    {'coins': '20,000', 'amount': '20000', 'price': 'Rp 2.000'},
    {'coins': '50,000', 'amount': '50000', 'price': 'Rp 5.000'},
    {'coins': '100,000', 'amount': '100000', 'price': 'Rp 10.000', 'tag': 'Popular'},
    {'coins': '200,000', 'amount': '200000', 'price': 'Rp 20.000'},
    {'coins': '500,000', 'amount': '500000', 'price': 'Rp 50.000', 'tag': 'Best Seller'},
  ];
  ...
  Widget _walletTabs() {
    const labels = ['Top Up', 'Transaction', 'Exchange', 'History'];
    const icons = [Icons.savings_rounded, Icons.receipt_long_rounded, Icons.swap_horiz_rounded, Icons.history_rounded];
  ...
  Widget _historyTab() => _transactionTab();
  ...
  Widget _exchangeTab() {
    final isCoinToDiamond = _exchangeMode == 0;
    final input = int.tryParse(_exchangeController.text.replaceAll(',', '').trim()) ?? 0;
    final output = isCoinToDiamond ? input : (input * 70 ~/ 100);
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Exchange', style: TextStyle(color: Color(0xFF70451E), fontSize: 22, fontWeight: FontWeight.w800)),
          const SizedBox(height: 6),
          const Text('Coin → Diamond 1:1 • Diamond → Coin 70%', style: TextStyle(color: Color(0xFF8F7960), fontSize: 12)),
          const SizedBox(height: 16),
          Row(
            children: [
              Expanded(child: _exchangeModeButton('Coin → Diamond', 0)),
              const SizedBox(width: 10),
              Expanded(child: _exchangeModeButton('Diamond → Coin', 1)),
            ],
          ),
          const SizedBox(height: 16),
          Container(
            ...
            child: Column(
              children: [
                TextField(
                  ...
                  decoration: InputDecoration(
                    labelText: isCoinToDiamond ? 'Jumlah Coin' : 'Jumlah Diamond',
                    hintText: 'Contoh: 1000',
                    prefixIcon: Icon(isCoinToDiamond ? Icons.monetization_on_rounded : Icons.diamond_rounded),
                    suffixText: 'min 100',
                ...
                Container(
                  ...
                  child: Text(
                    'Hasil: ${_formatCoins(output)} ${isCoinToDiamond ? 'Diamond' : 'Coin'}',
                    textAlign: TextAlign.center,
                    style: const TextStyle(color: Color(0xFF70451E), fontWeight: FontWeight.w800),
                  ),
                ),
                const SizedBox(height: 14),
                SizedBox(
  ...
  Widget _exchangeModeButton(String title, int mode) {
    final active = _exchangeMode == mode;
    return InkWell(
      borderRadius: BorderRadius.circular(16),
      onTap: () => setState(() {
        _exchangeMode = mode;
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: active ? const Color(0xFFFFE7AF) : Colors.white,
          borderRadius: BorderRadius.circular(16),
          border: Border.all(color: active ? const Color(0xFFD9A83B) : const Color(0xFFEBD9B9)),
        ),
  ...
  void _doExchange() {
    ...
    if (value == null || value < 100 || value % 100 != 0) {
      _showMessage(context, 'Jumlah harus kelipatan 100 dan minimal 100.');
      return;
    }

    if (_exchangeMode == 0) {
      if (value > testerCoinBalance.value) {
        _showMessage(context, 'Coin tidak cukup.');
        return;
      }
      _setTesterCoins(testerCoinBalance.value - value);
      _addTesterDiamonds(value);
      _addTesterTransaction('Exchange -${_formatCoins(value)} Coin → +${_formatCoins(value)} Diamond');
      _exchangeController.clear();
      setState(() {});
      _showMessage(context, 'Exchange berhasil.');
    } else {
      ...
      _setTesterDiamonds(testerDiamondBalance.value - value);
      _addTesterCoins(coins);
      _addTesterTransaction('Exchange -${_formatCoins(value)} Diamond → +${_formatCoins(coins)} Coin');
      _exchangeController.clear();
      setState(() {});
      _showMessage(context, 'Exchange berhasil.');
    }
...
class _WalletPageState extends State<WalletPage> {
  ...
  final List<Map<String, String>> _packages = const [
    {'coins': '10,000', 'price': 'Rp 1.000'},
    {'coins': '20,000', 'price': 'Rp 2.000'},
    {'coins': '50,000', 'price': 'Rp 5.000'},
    {'coins': '100,000', 'price': 'Rp 10.000', 'tag': 'Popular'},
    {'coins': '200,000', 'price': 'Rp 20.000'},
    {'coins': '500,000', 'price': 'Rp 50.000', 'tag': 'Best Seller'},
  ];
      ...
      body: SafeArea(
        child: CustomScrollView(
          slivers: [
            ...
            SliverToBoxAdapter(child: _walletTabs()),
            if (_tab == 0)
              SliverToBoxAdapter(child: _topUpSection())
            else
              SliverFillRemaining(
                hasScrollBody: false,
                child: _WalletPlaceholder(
                  title: ['Top Up', 'Transaction', 'Exchange', 'History'][_tab],
                ),
              ),
          ],
        ),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          _goldIconButton(Icons.help_outline_rounded, () {
            _showMessage(context, 'Wallet dan transaksi kamu ada di sini.');
          }),
        ],
      ),
    );
                ...
                child: Column(
                  children: [
                    ...
                    Text(
                      ...
                      style: TextStyle(
                        color: active
                            ? const Color(0xFF70451E)
                            : const Color(0xFF8F806E),
                        fontSize: 12,
                        fontWeight: active ? FontWeight.w800 : FontWeight.w600,
                      ),
                    ),
    ...
    return Stack(
      ...
      children: [
        Material(
          ...
          child: InkWell(
            borderRadius: BorderRadius.circular(22),
            onTap: () => _showMessage(context, 'Pilih paket $coins Coin — $price'),
            child: Container(
              constraints: const BoxConstraints(minHeight: 166),
              padding: const EdgeInsets.fromLTRB(14, 18, 14, 14),
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(
                    color: const Color(0xFF8D6328).withOpacity(.08),
                    blurRadius: 14,
  ...
  Widget _promoCard() {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [Color(0xFFFFF7E3), Color(0xFFFFEBC0)],
        ),
        borderRadius: BorderRadius.circular(24),
        border: Border.all(color: const Color(0xFFE0B65B)),
      ),
          ...
          const Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 3),
                Text(
                  'Dapatkan lebih banyak coin dengan harga terbaik.',
                  style: TextStyle(
                    color: Color(0xFF96754D),
                    fontSize: 12,
                    height: 1.25,
                  ),
                ),
          ...
          Padding(
            ...
            child: GridView.count(
              ...
              children: [
                ...
                _ProfileShortcut(Icons.verified_rounded, 'Badge',
                    () => Navigator.push(context,
                        MaterialPageRoute(builder: (_) => const BadgePage()))),
                _ProfileShortcut(Icons.workspace_premium_rounded, 'VIP',
                    () => Navigator.push(context,
...
class _ChatPageState extends State<ChatPage> {
  ...
  final _friends = const [
    ('Nana', '1000002'), ('Raka', '1000003'), ('Salsa', '1000004'),
  ];
