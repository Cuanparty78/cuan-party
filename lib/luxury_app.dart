...
class _LuxuryBackdrop extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return IgnorePointer(
      child: CustomPaint(
        painter: _PalaceBackgroundPainter(),
...
class _PalaceBackgroundPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = -2; i < 7; i++) {
      final path = Path()..moveTo(i * 95.0, h * .20);
      path.cubicTo(
        i * 95.0 + 55, h * .35,
        i * 95.0 - 45, h * .52,
        i * 95.0 + 70, h * .70,
      ...
      canvas.drawPath(path, marble);
    }

    // Palace side columns.
    _drawColumn(canvas, x: 28, width: 44, height: h);
    _drawColumn(canvas, x: w - 72, width: 44, height: h);

    // Central grand arch.
    final archPaint = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 8
      ..color = const Color(0xA8FFF0CE);
    final arch = Path()
      ..moveTo(w * .19, h * .68)
      ..lineTo(w * .19, h * .22)
      ..quadraticBezierTo(w * .19, h * .055, w * .5, h * .045)
      ..quadraticBezierTo(w * .81, h * .055, w * .81, h * .22)
      ..lineTo(w * .81, h * .68);
    canvas.drawPath(arch, archPaint);

    // Inner gold arch lines.
    final inner = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 2
      ..color = const Color(0xB8D9A34F);
    for (int i = 0; i < 3; i++) {
      final inset = i * 12.0;
      final p = Path()
        ..moveTo(w * .19 + inset, h * .68)
        ..lineTo(w * .19 + inset, h * (.22 + i * .012))
        ..quadraticBezierTo(w * .19 + inset, h * (.07 + i * .006), w * .5, h * (.045 + i * .008))
        ..quadraticBezierTo(w * .81 - inset, h * (.07 + i * .006), w * .81 - inset, h * (.22 + i * .012))
        ..lineTo(w * .81 - inset, h * .68);
      canvas.drawPath(p, inner);
    }

    // Chandelier glow + hanging crystals.
    final glow = Paint()
      ..shader = RadialGradient(
        colors: [const Color(0xFFFFE7A1).withOpacity(.62), Colors.transparent],
      ).createShader(Rect.fromCircle(center: Offset(w * .5, h * .09), radius: 145));
    canvas.drawCircle(Offset(w * .5, h * .09), 145, glow);
    final gold = Paint()..color = const Color(0xFFE6B95E);
    canvas.drawRect(Rect.fromLTWH(w * .5 - 2, 0, 4, h * .065), gold);
    for (int i = -4; i <= 4; i++) {
      final x = w * .5 + i * 17.0;
      final y = h * .07 + (i.abs() * 4);
      canvas.drawCircle(Offset(x, y), i == 0 ? 5 : 3, gold);
      canvas.drawLine(Offset(x, y), Offset(x, y + 20 + i.abs() * 3), gold..strokeWidth = 1);
    }

    // Warm light pools.
    final pool = Paint()
      ..shader = RadialGradient(
        colors: [const Color(0xFFFFE8A8).withOpacity(.26), Colors.transparent],
      ).createShader(Rect.fromCircle(center: Offset(w * .5, h * .55), radius: w * .65));
    canvas.drawCircle(Offset(w * .5, h * .55), w * .65, pool);

    // Marble palace floor / medallion.
    final floorY = h * .82;
    final floor = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = 2.2
      ..color = const Color(0x66A46B27);
    canvas.drawOval(Rect.fromCenter(center: Offset(w * .5, floorY), width: w * 1.25, height: h * .23), floor);
    canvas.drawOval(Rect.fromCenter(center: Offset(w * .5, floorY), width: w * .72, height: h * .13), floor);
    final star = Path();
    for (int i = 0; i < 8; i++) {
      final a = i * 3.14159 / 4;
      final r = i.isEven ? 75.0 : 26.0;
      final pt = Offset(w * .5 + r * math.cos(a), floorY + r * .38 * math.sin(a));
      if (i == 0) star.moveTo(pt.dx, pt.dy); else star.lineTo(pt.dx, pt.dy);
    }
    star.close();
    canvas.drawPath(star, floor);
  }

  void _drawColumn(Canvas canvas, {required double x, required double width, required double height}) {
    final body = Paint()
      ..shader = const LinearGradient(
        colors: [Color(0xFFEAD7B7), Color(0xFFFFF7E7), Color(0xFFD2A05A)],
      ).createShader(Rect.fromLTWH(x, 0, width, height));
    canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(x, height * .18, width, height * .66), const Radius.circular(14)), body);
    final gold = Paint()..color = const Color(0xB8B77A32);
    for (final y in [height * .16, height * .23, height * .79, height * .86]) {
      canvas.drawRRect(RRect.fromRectAndRadius(Rect.fromLTWH(x - 8, y, width + 16, 9), const Radius.circular(5)), gold);
    }
  }
